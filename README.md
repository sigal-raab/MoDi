...
Here is an example of running evaluation for a model saved in `<model_ckpt>`, where `<gt_data>` is the path to the data the action recognition model was trained with.
`--motions` is the number of motions that will be generated by the model for the evaluation.
~~~bash
python evaluate.py --motions 2000 --ckpt <model_ckpt> --act_rec_gt_path <gt_data>
~~~
